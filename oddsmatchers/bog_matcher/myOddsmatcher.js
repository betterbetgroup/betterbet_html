...
const SHOW_DATE_AND_TIME_WIDTH = 1000;
...
go_to_input_and_update_global_for_the_input(filterId) {
    ...
    switch (filterId) {
        ...
        case 'min-potential-profit':
            globalFilters.minROI = parseFloat(this.shadowRoot.getElementById('min-potential-profit').value) || null;
            break;
...
get_selected_filter_name() {

    return filtersDropdown = this.shadowRoot.querySelector('#filters-select').value;
...
confirm_filter_name() {
    ...
    let obj = this.append_global_filters_to_options(filter_name);
    ...
    if (obj.is_delete) {
        ...
        this.removeOptionFromDropdown(filter_name);
...
sort_data(sort_by, method) {
        ...
        if (sort_by == 'rating') {
            ...
            globalData = this.sort_rows_by_ROI(globalData, method)
