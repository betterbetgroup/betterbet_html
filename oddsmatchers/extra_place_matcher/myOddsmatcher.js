...
const LARGE_BOOKMAKER_AND_ODDS = 1355;

const SHOW_PLACES = 1270; 
const MEDIUM_FONT_WIDTH = 1190;
const SHOW_DATE_AND_TIME_WIDTH = 1100;
const SHOW_INFO_WIDTH = 1030;
...
class extraPlaceOddsmatcher extends HTMLElement {
    ...
    sort_rows_by_ImpliedOdds(rows, method) {
...
`;
    ...
    create_row(row) {
        ...
        let win_exchange_image = this.get_exchange_image(row.win_exchange)
        let place_exchange_image = this.get_exchange_image(row.place_exchange)
...
add_lock_if_premium() {
    ...
    go_to_input_and_update_global_for_the_input(filterId) {
        ...
        switch (filterId) {
            ...
            case 'min-Implied Odds':
                globalFilters.minImpliedOdds = parseFloat(this.shadowRoot.getElementById('min-Implied Odds').value) || null;
                break;
            case 'max-Implied Odds':
                globalFilters.maxImpliedOdds = parseFloat(this.shadowRoot.getElementById('max-Implied Odds').value) || null;
                break;
            case 'min-places':
                globalFilters.minPlaces = parseFloat(this.shadowRoot.getElementById('min-places').value) || null;
                break;
            case 'min-extra_places':
                globalFilters.minExtraPlaces = parseFloat(this.shadowRoot.getElementById('min-extra_places').value) || null;
                break;
            ...
            case 'min-potential-profit':
                globalFilters.minPotentialProfit = parseFloat(this.shadowRoot.getElementById('min-potential-profit').value) || null;
                break;
    ...
    sort_data(sort_by, method) {
            ...
            if (sort_by == 'Implied Odds') {
                globalData = this.sort_rows_by_ImpliedOdds(globalData, method)
    ...
    handleResize() {
        ...
        this.check_if_removing_or_showing_places(width);
    ...
    set_font_size(width) {
        ...
        if (width < LARGE_FONT_WIDTH) {
            ...
            expected_profit_header_width = '145px';
        ...
        this.shadowRoot.querySelector('#extra_places_header').style.width = places_width;
        ...
        back_odds_values.forEach(function(back_odds_value) {
            back_odds_value.style.fontSize = data_font_size;
        });
        ...
        var at_symbols = this.shadowRoot.querySelectorAll('.at_symbol, .and_symbol');
    ...
    make_odds_and_platform_small_if_screen_small(width) {
        ...
        const and_symbol = this.shadowRoot.querySelectorAll('.and_symbol');
    ...
    check_if_removing_or_showing_places(width) {

        let places_display = 'table-cell';

        if (width < SHOW_PLACES) {
            places_display = 'none';
        }

        this.shadowRoot.querySelector('#extra_places_header').style.display = places_display;

        var date_and_time_values = this.shadowRoot.querySelectorAll('.extra_places_data');
        date_and_time_values.forEach(function(date_and_time_value) {
            date_and_time_value.style.display = places_display;
        });
