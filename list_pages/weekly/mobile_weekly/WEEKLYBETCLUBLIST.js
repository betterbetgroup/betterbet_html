...
class ProfitTracker extends HTMLElement {
    ...
    process_new_final_data(data) {
        ...
        if (data.user_suo_object) {
            ...
            if (user_suo_object.length == 0) {
                ...
                this.change_available_status_on_object()
            }
    ...
    change_available_status_on_object() {

        const currentDate = new Date();
        const sevenDaysAgo = new Date(currentDate.getTime() - 7 * 24 * 60 * 60 * 1000);

        user_suo_object.forEach(obj => {
    ...
    get_availability_text(bookmaker) {
        const bookmakerData = user_suo_object.find(b => b.bookmaker_name === bookmaker);
        if (!bookmakerData) {
            return 'Now';
        }
    ...
    create_row(row) {
        ...
        if (!is_available) {
            availability_text = this.get_availability_text(row.bookmaker);
...
alternateText() {
    ...
    appendRows() {
        ...
        this.change_availability_text_colour();
    ...
    sort_filtered_data() {
        ...
        switch (current_sort) {
            case 'profit':
                filteredData.sort((a, b) => parseFloat(b.profit.replace('Varies', '0').replace('£', '')) - parseFloat(a.profit.replace('Varies', '0').replace('£', '')));
                break;
            case 'a-z':
    ...
    change_availability_text_colour() {
        
        let promo_info_texts = this.shadowRoot.querySelectorAll('.promo_info');
    ...
    render() {
        return fetch('https://betterbetgroup.github.io/betterbet_html/list_pages/weekly/mobile_weekly/z.html')
        //return fetch('z.html')
...
customElements.define('mobile-weekly', ProfitTracker);
